...
bool Drv8301::config(float requested_gain, float* actual_gain) 
{
    ...
    new_config.control_register_1 = (21 << 6) | // Overcurrent set to approximately 150A at 100degC. This may need tweaking.
                                    (0b01 << 4) | // OCP_MODE: latch shut down
                                    (0b00 << 3) | // 6x PWM mode
                                    (0b00 << 2) | // don't reset latched faults
                                    (0b00 << 0); // gate-drive peak current: 1.7A

    new_config.control_register_2 = (0b00 << 6) | // OC_TOFF: cycle by cycle
