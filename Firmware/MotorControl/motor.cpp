...
float Motor::effective_current_lim() 
{
    ...
    {
        //gimbal motor is voltage control
...
void Motor::current_meas_cb(uint32_t timestamp, std::optional<Iph_ABC_t> current) 
{
    ...
    if (control_law_) 
    {
        Error err = control_law_->on_measurement(vbus_voltage,
                            current_meas_.has_value() ? std::make_optional(std::array<float, 3>{current_meas_->phA, current_meas_->phB, current_meas_->phC}) : std::nullopt,
                            timestamp);
        if (err != ERROR_NONE) 
        {
...
void Motor::pwm_update_cb(uint32_t output_timestamp) 
{
    ...
    if (control_law_) 
    {
        control_law_status = control_law_->get_output(output_timestamp, pwm_timings, &i_bus);
    }
